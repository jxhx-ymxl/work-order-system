...
| F1-4 | AI 自动分类与优先级 | **B** | As a 提交人 / I want 只写标题和内容，类型与优先级由系统判断 / so that 高峰期我不必逐项选、也不会因选错类型拿到错误的处理时限 | WHEN 未提供 `type` 或 `priority` THEN 系统 SHALL 调用 triage 取建议值并落库，且提交响应 SHALL NOT 因该调用阻塞<br>WHEN triage 不可用或返回非法值 THEN 系统 SHALL 使用兜底值（`OTHER`/普通）落库且提交 SHALL 成功<br>WHEN 异步结果写回且类型或优先级发生变化 THEN 系统 SHALL 按下方 H4 规则重算 `sla_deadline`、写一条修正日志，且 SHALL NOT 覆盖用户手工填写的值<br>**WHEN 工单处于待分诊状态 THEN 界面 SHALL 显示"分类中"而不是兜底值（未完成，缺数据源：`triage_status` 未在后端 VO 中暴露，见 D61）** | **已放开（P5 步骤 3，2026-09-25）**：`OrderCreateView.vue` 的 `type`/`priority` 均可选（不选则不放进请求体），triage 可被真实触发；**但"分类中"标记尚无法渲染**——后端 VO 未暴露 `triage_status` | MQ（复用 F5-4） | 是 | |
