...
| 代码事实 | 位置 | 文档状态 | 标记 |
| --- | --- | --- | --- |
| ~~AI Triage 只在 `type` 或 `priority` 为空时触发，失败回退 `OTHER`/普通~~ **已修复（P5 步骤 1 + 步骤 3，2026-09-25）**：触发条件仍是"缺 `type`/`priority`"，但①提交不再同步调 LLM（P5 步骤 1）；②**失败不再回退成一次"成功的判断"**——`triage()` 改为抛 `TriageUnavailableException`，消费端据此走重试账本，阶梯走完则工单收口 `FAILED`（P5 步骤 3，见 D62 / I12） | `WorkOrderServiceImpl.submitOrder`、`OrderTriageServiceImpl.triage`、`OrderTriageConsumeService` | `CONTEXT.md` 描述了 Triage 未说明触发条件 → 现已由 F1-4 的验收标准与「提交通知规则」旁的 H4/三态规则覆盖 | 【实现】 |
...
| F1-4 | AI 自动分类与优先级 | **B** | As a 提交人 / I want 只写标题和内容，类型与优先级由系统判断 / so that 高峰期我不必逐项选、也不会因选错类型拿到错误的处理时限 | WHEN 未提供 `type` 或 `priority` THEN 系统 SHALL 调用 triage 取建议值并落库，且提交响应 SHALL NOT 因该调用阻塞<br>WHEN triage 不可用或返回非法值 THEN 系统 SHALL 使用兜底值（`OTHER`/普通）落库且提交 SHALL 成功、`triage_status` SHALL 保持 `PENDING` 并 SHALL 写重试账本<br>WHEN 异步结果写回且类型或优先级发生变化 THEN 系统 SHALL 按下方 H4 规则重算 `sla_deadline`、写一条修正日志，且 SHALL NOT 覆盖用户手工填写的值<br>**WHEN 重试阶梯走完仍未成功 THEN 系统 SHALL 把 `triage_status` 置为 `FAILED`（与账本转 `PARKED` 同事务）**<br>**WHEN 工单处于待分诊/分诊失败 THEN 界面 SHALL 显示"分类中"/"分类失败"，而不是把兜底值当成人机判断结果** | **已完成（P5 步骤 3，2026-09-25）**：`OrderCreateView.vue` 的 `type`/`priority` 均可选（不选则不放进请求体）；`triage_status` 已随**提交/列表/详情**三处响应返回，前端按 `TRIAGE_STATUS_MAP` 渲染三态。本轮同时修掉三个缺陷（两处 `toVO` 漏字段、PARKED 不置 FAILED、**LLM 失败被洗成一次成功**），见 D62 | MQ（复用 F5-4） | 是 | |
