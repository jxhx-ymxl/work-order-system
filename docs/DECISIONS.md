...
**已用标题查询确认**（`SELECT id,title FROM t_work_order WHERE id IN (888,902)`）——若实际落点与推断不符，以查询结果为准。
...
> ⚠ **待贴**：正文与逐条明细属于"经审视的准确率"的**唯一原始凭证**（服务器上的文件迟早会丢）。
> 本机拿不到这两个文件（不在本机文件系统、也不在仓库里），**不代为转述、更不虚构**——
> 请把 `~/eval-pass1.txt` 全文贴回（或拷进仓库外的临时目录让我读）。
>
> 已由委托方给出的**摘要**（转述，非逐条原始输出）：类型 14/14、优先级 8/8、信息不足组保守率 6/6、
> 三档计数 0/0/0、失败清单 0 条，总耗时 4.0–14.1s（最长 E4 = 14.1s）。

### 附录 B · `~/eval-pass2.txt`（`--reverse`）原始输出

> ⚠ **待贴**：同上。摘要（转述）：与第一遍完全一致——类型 14/14、优先级 8/8、保守 6/6、失败清单 0 条。
