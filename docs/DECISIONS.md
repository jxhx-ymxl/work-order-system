...
- **数据来源（必须写清）**：**服务器真机 + 真模型**，正向/反向两遍（`python scripts/triage-eval.py` 与 `--reverse`）。
  本机无 LLM key（`$env:LLM_API_KEY` 为空、`deploy/.env` 只有 MySQL/RabbitMQ），**因此本轮的数字由服务器侧产出、
  本机未独立复验**；D68 是这批数字的唯一留档地。
...
**与用例的对应关系**：按工单 id ↔ 用例的映射**推断**为 **E4**（长文本，最吃读超时）与 **I6**（信息不足组）。
委托方表示已用标题查询核对过（`SELECT id,title FROM t_work_order WHERE id IN (888,902)`），
但**该查询的原始结果尚未随材料提供**——所以这里**仍按"推断"标注，不写成"已确认"**。
若查询结果与推断不符，**直接按实际改写这一句**：推断就是推断，凭证不足时不能用"已确认"把它抬成事实。
...
- **历史残留（登记为 P7 收尾项，本轮不清）**：
  ...
  · **例外：`888` 与 `902` 明确保留，不删**——它们是"失败 → 账本 → 重试自愈"的**实测凭证**，
    删掉就只剩文档里的一句话了。
  · **`902` 的当前结局**：⏳ **待填**（三选一：仍 `PENDING attempt=5` / 已 `SUCCEEDED` / 已 `PARKED`）
    ＋ **查询时刻**：⏳ **待填**（`SELECT event_id,attempt,status,last_error FROM t_message_retry WHERE event_id LIKE 'order:902:%';`）。
    **为什么必须一起写**：只写"它是凭证"而不写它现在停在哪，等于把凭证的读数留空；
    而且 `PARKED` 与 `SUCCEEDED` 对"兜底是否生效"的结论方向相反。

### 附录 A · `~/eval-pass1.txt` 原始输出

**来源**：服务器 `/root/eval-pass1.txt`，由用户 `cat` 后粘贴。

**⚠ 粘贴中丢失的那一行**：`===== eval-pass1.txt =====` 是 `echo` 打出来的**分隔行、不属于文件内容**——
本附录**不补造**它；文件身份由上面这行"来源"承担。

```
⏳ 待贴正文（41 行）。已知锚点（便于核对粘贴是否完整，仍不代为转述内容）：
   · 行数：41
   · 首行：登录成功；用例 20 条（正向顺序）…
   · 末行：DELETE FROM t_work_order     WHERE id          IN (949,…,968);
   · 含本次 40 个 id 中的前 20 个（949–968）与 6 张表的清理 SQL
   · 粘贴要求：整段原样贴入，不重排、不删行（它是"经审视的准确率"的唯一原始凭证）
```

> 已由委托方给出的**摘要**（转述，**不是**逐条原始输出）：类型 14/14、优先级 8/8、信息不足组保守率 6/6、
> 三档计数 0/0/0、失败清单 0 条，总耗时 4.0–14.1s（最长 E4 = 14.1s）。

### 附录 B · `~/eval-pass2.txt`（`--reverse`）原始输出

**来源**：服务器 `/root/eval-pass2.txt`，由用户 `cat` 后粘贴。

**⚠ 同上**：`===== eval-pass2.txt =====` 那行也是 `echo` 的分隔行，**不补造**。

```
⏳ 待贴正文（41 行）。已知锚点：
   · 行数：41
   · 首行：登录成功；用例 20 条（反向顺序）…
   · 末行：DELETE FROM t_work_order     WHERE id          IN (969,…,988);
   · 含本次 40 个 id 中的后 20 个（969–988）与 6 张表的清理 SQL
```

> 摘要（转述，**不是**逐条原始输出）：与第一遍完全一致——类型 14/14、优先级 8/8、保守 6/6、失败清单 0 条。
