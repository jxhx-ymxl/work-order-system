...
python scripts/triage-eval.py --base-url http://127.0.0.1:9000          # 默认等待上限 150s（首次重试退避 1 分钟）
```
...
| 三档计数（分开报，别混成一个"失败清单"） | **判错**（有结论但与期望不符）/ **超时未完成**（等待上限内一直 `PENDING`）/ **分诊失败**（`triageStatus=FAILED`，阶梯走完） |
| **对外口径** | 计准确率时**超时与分诊失败都按失败计、且保留在分母里（不缩分母）**，例如 `13/14 = 92.9%`；信息不足组未判定按"不保守"计，分母保持整组 6 |
...
**已跑（2026-09-26，服务器真机 + 真模型，正向/反向两遍）：类型 14/14、优先级 8/8、信息不足组 6/6、失败清单 0 条，三档计数 0/0/0；明细见 D68。**
...
| --- | --- | --- | --- | --- |
| **改前基线**（真机跑分，2026-09-25，**O2/E2 按双值口径**） | 旧 prompt + 旧期望值 | **85.7%（12/14）** | 见失败清单 | **4/6** |
| 改 prompt 后（真机两遍，2026-09-26） | 新 prompt（四类边界 + 保守规则）+ **O2/E2 已收窄为单值** | **14/14（两遍一致）** | **8/8** | **6/6** |

> **两项门槛已在 2026-09-26 调整**（见 D66）：`llm.api.timeout` **5000 → 15000ms**（真机各条端到端 2.0–9.1s（改前）
> → **4.0–14.1s（改后 40 次调用，最长是 E4 长文本 14.1s，距 15s 读超时约 2s 余量）**，
> 5s 落在分布中部 → 长输入偶发失败）；评测脚本等待上限 **90 → 150s**（≥ 首次重试退避 1 分钟），
> 并把"判错 / 超时未完成 / 分诊失败"分开计数、**超时不再从分母里消失**。
> 代价：最坏 ≈30s 才降级、启动自检最坏多等 10s、消费端在事务里占连接的时间从 5s 变 15s（并发=1 时可接受，调大并发前要重算）。
>
> > 已被 D67 取代：LLM 调用自 2026-09-26 起在事务外。"占连接 5s→15s"与"调大并发前要重算连接"均不再适用，新约束见 D67。

逐条判读（改前）：**O1 = 真实模型错**（期望值不改）、**D2 = 边界模糊**（改 prompt 不改期望值）、**I4/I6 = 可识别的倾向**（保守规则的依据）。
...
（这里的 21 是"顶在池上限不动"＝池被打满；§5.5/D67 说"判定不了"的是池有余量时 8–11 那种峰值，两者不矛盾。）
