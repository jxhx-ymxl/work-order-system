...
| 顺序 | 阶段 | 内容 |
| --- | --- | --- |
| 1 | **P4 幂等 / 死信 / 退避** | ~~`t_consume_record` 消费去重表~~ **已完成（P4 步骤 1，2026-09-25）**：与业务写同事务插入，重复投递命中去重键直接 ACK（见 D52）；**仍待做**：DLX + 停车队列、`t_message_retry` 与 1m/5m/15m/1h/6h 阶梯退避、**把消费者 ERROR 分支从 ACK 改成 NACK**（当前 ACK 是因为还没有 DLX）、扫描 SQL 直接排除已通知工单（替掉 SLA 调度器里的 Redis 粗粒度去重）、通知表回填 `ref_type/ref_id`（当前 82/82 全 NULL，第二道防线建不起来） |
